...
<div class="container-fluid px-0 mx-0 content-2 rounded">
    <div class="card shadow px-0">
        <div class="card-header border border-1 d-flex align-items-center justify-content-between">
            ...
            <a href="<?= site_url('/mahasiswa/profile'); ?>" class="text-danger"><i class="fa-solid fa-xmark"></i></a>
        </div>
        ...
        <div class="card-body">
            <?php if ($profile) { ?>
                <form action="<?= site_url('/mahasiswa/profile/update'); ?>" method="post" enctype="multipart/form-data">
                    <input type="hidden" name="id_mhs" id="id_mhs" value="<?= $profile['id_mhs']; ?>">
                    <input type="hidden" name="id_user" id="id_user" value="<?= $profile['id_user']; ?>">
                    <div class="row">
                        <div class="col-lg-6 col-md-6">
                            <div class="form-group">
                                <label for="id_user" class="form-label">ID User:</label>
                                <input type="text" class="form-control" id="id_user" name="id_user" value="<?= $profile['id_user']; ?> - <?= $profile['nama_lengkap']; ?>" readonly>
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                <label for="jenis_kelamin" class="form-label">Jenis
                                    Kelamin:</label>
                                <select class="form-control" name="jenis_kelamin" id="jenis_kelamin" value="<?= $profile['jenis_kelamin']; ?>">
                        ...
                        <div class=" col-lg-6 col-md-6">
                            <div class="form-group">
                                <label for="program_studi" class="form-label">Program
                                    Studi:</label>
                                <select class="form-control" name="program_studi" id="program_studi">
                                    <option selected disabled>-- Program Studi ---</option>
                                    <option value="S1-Teknik Informatika" <?= $profile['program_studi'] == 'S1-Teknik Informatika' ? 'selected' : ''; ?>>
                                        S1-Teknik Informatika</option>
                                    <option value="S1-Sistem Informasi" <?= $profile['program_studi'] == 'S1-Sistem Informasi' ? 'selected' : ''; ?>>
                                        S1-Sistem Informasi</option>
                                    <option value="S1-Sistem Digital" <?= $profile['program_studi'] == 'S1-Sistem Digital' ? 'selected' : ''; ?>>
                                        S1-Sistem Digital</option>
                                </select>
                            </div>
                            <div class="form-group">
                                <label for="tahun_angkatan">Tahun Angkatan:</label>
                                <input type="text" class="form-control" id="tahun_angkatan" name="tahun_angkatan" value="<?= $profile['tahun_angkatan']; ?>">
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <div class="custom-file">
                                    <input type="file" class="custom-file-input <?= session('errors.upload_foto') ? 'is-invalid' : null ?>" id="upload_foto" name="upload_foto" value="<?= $profile['upload_foto']; ?>">
                                    <?php if (session('errors.upload_foto')) : ?>
                                        <div class="invalid-feedback">
                                            <?= session('errors.upload_foto') ?>
                                        </div>
                                    <?php endif ?>
                                    <label class="custom-file-label" for="upload_foto"><?= $profile['upload_foto']; ?></label>
                                </div>
                                <div class="card shadow mt-1" style="width: 5rem;">
                                    <img src="<?= base_url('uploads/' . $profile['upload_foto']); ?>" class="card-img" alt="Foto Profil">
                                </div>
                            </div>
